<template>
  <Teleport to="body">
    <section class="">
      <Transition enter-from-class="opacity-0" enter-active-class="transition-all duration-300 ease-out"
        enter-to-class="opacity-100" leave-from-class="opacity-100"
        leave-active-class="transition-all duration-300 ease-in" leave-to-class="opacity-0">
      ...
      <Transition enter-from-class="opacity-0 scale-90" enter-active-class="transition-all duration-300 ease-out"
        ...
        leave-active-class="transition-all duration-300 ease-in" leave-to-class="opacity-0 scale-90">
        <div v-if="show" @click="closeContact" class="fixed top-0 left-0 h-screen w-full z-50 overflow-y-auto">
          <div class="container min-h-screen py-16 flex place-items-center">
            <div id="content" class="max-w-5xl w-full mx-auto p-8 lg:p-10 bg-brand-light rounded-2xl">
              <h2 class="text-3xl text-headline font-semibold text-center xl:text-4xl/[3rem] mb-8 xl:mb-10">
                Contact Information
              </h2>
              <form action="" id="contacForm" class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:gap-8"
                @submit.prevent="saveContact">
                <div>
                  <BaseInput v-model.trim="contact.name" label="First Name" type="text" :error="errors.name" id="name"
                    @cancel-error="cancelError('name')"></BaseInput>
                </div>
                <div>
                  <BaseInput v-model.trim="contact.surname" label="Last Name" type="text" :error="errors.surname"
                    id="surname" @cancel-error="cancelError('surname')"></BaseInput>
                </div>
                <div>
                  <BaseInput v-model="contact.phone" label="Mobile Phone" type="tel" :error="errors.phone" id="phone"
                    @cancel-error="cancelError('phone')"></BaseInput>
                </div>
                <div>
                  <BaseInput v-model.trim="contact.email" label="E-mail" type="email" :error="errors.email" id="email"
                    @cancel-error="cancelError('email')"></BaseInput>
                </div>
                <div>
                  <DropDown v-model="contact.department" :items="departments" label="Department"
                    :error="errors.department" id="department" @cancel-error="cancelError('department')">
                  </DropDown>
                </div>
                <div>
                  <BaseCheckbox v-model="contact.privacy" label="I Agree to Terms and Conditions and Privacy Policy"
                    :error="errors.privacy" id="privacy" @cancel-error="cancelError('privacy')"></BaseCheckbox>
                </div>
